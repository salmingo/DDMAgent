...
namespace DDMAgent
{
	class AstroArith
	{
		...
		public double Epoch2JC(double epoch)
		{
            return (epoch - 2000.0) * 0.01;
		}
		...
		public bool StringDegree2Double(string strDegree, ref double dblDegree)
		{
			...
			for (int i = 0; i < nLen; i++) // 遍历字符串
			{
				...
				if (ch >= '0' && ch <= '9')// 是数字
				{
					...
					{
						cnt++;
						if (part < 2 && cnt > 2) // 在度分区出现了连续的多于两个数字
						{
		...
		public void Eq2AltAzi(double ha, double dec, ref double azi, ref double alt)
		{
			...
			sinz = Sin(m_vSiteLat) * Sin(dec) + Cos(m_vSiteLat) * Cos(dec) * Cos(ha);
			cosz = Sqrt(1 - sinz * sinz);
			alt = Atan2(sinz, cosz);

			sina = Cos(dec) * Sin(ha);
			cosa = Sin(m_vSiteLat) * Cos(dec) * Cos(ha) - Cos(m_vSiteLat) * Sin(dec);
			azi = reduce_angle(Atan2(sina, cosa) + PI);
		}
         ...
         *  大圆距离, 量纲: 弧度
         **/
        public double SphereAngle(double l1, double b1, double l2, double b2)
        {
            return Acos(sind(b1) * sind(b2) + cosd(b1) * cosd(b2) * cosd(l1 - l2));
        }
    }
}
